...
describe("Last.Fm - Helpers Module", () => {
  ...
  describe(".addSignatureToUrl", () => {
    it("should add API signature to given URL", () => {
      const url = helpers.getBaseUrl("track.love");

      url.searchParams.set("artist", "KITANO REM");
      url.searchParams.set("track", "RAINSICK");
      url.searchParams.set("sk", "YOUR_SESSION_KEY");

      helpers.addSignatureToUrl(url);

      expect(url.searchParams.get("api_sig")).to.be.equal(
